...
export class BridgeBracedFrames {
    ...
    randomize(landscape) {
        ...
        this.main_brace_type = main_brace_type_option[this.rand.rint(0, main_brace_type_option.length)];
        ...
        if (this.rand.rbool() && this.project.bridge_support.height_over_span === 0 && num_elements < 2000) {
            for (let x = 0; x < this.area_width; x++) {
                for (let y = 0; y < this.area_height; y++) {
    ...
    draw(ctx, debug = false) {
        ...
        this.project.bridge_support.pier_top_pos.forEach(top_pos => {
            ...
            for (let x = 0; x < this.area_width; x++) {
                for (let y = 0; y < this.area_height; y++) {
                    let pos = new Pos(top_pos.x - (grid_width / 2) + (grid_size * x), top_pos.y + (grid_size * y));
                    ...
                    if (this.area[x][y] != ' ') {
                        ...
                        ctx.rect(pos.x, pos.y, grid_size, grid_size);
                        ctx.stroke();
    ...
    draw_x_brace(ctx, pos, grid_size) {
        ctx.beginPath();
        ctx.setLineDash([]);
    ...
    draw_v_brace(ctx, pos, grid_size) {
        ctx.beginPath();
        ctx.setLineDash([]);
        ctx.strokeStyle = this.inner_color.rgb();
        ctx.moveTo(pos.x, pos.y);
    ...
    draw_upsidedown_v_brace(ctx, pos, grid_size) {
        ctx.beginPath();
        ctx.setLineDash([]);
        ctx.strokeStyle = this.inner_color.rgb();
        ctx.moveTo(pos.x, pos.y + grid_size);
    ...
    draw_frontslash_brace(ctx, pos, grid_size) {
        ctx.beginPath();
        ctx.setLineDash([]);
        ctx.strokeStyle = this.inner_color.rgb();
        ctx.moveTo(pos.x, pos.y + grid_size);
    ...
    draw_backslash_brace(ctx, pos, grid_size) {
        ctx.beginPath();
        ctx.setLineDash([]);
        ctx.strokeStyle = this.inner_color.rgb();
        ctx.moveTo(pos.x, pos.y);

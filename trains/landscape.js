...
export class Landscape {
    ...
    randomize() {
        ...
        this.terrain[0] = new rPos(this.project, this.rand, this.start_pos.x + side_buffer, this.start_pos.y);
        this.terrain[1] = new rPos(this.project, this.rand, this.end_pos.x - side_buffer, this.end_pos.y);
        for (let i = 0; i < this.num_terrain_points; i++) {
            this.terrain[i + predefined_points] = new rPos(this.project, this.rand);
            this.terrain[i + predefined_points].random([side_buffer, this.project.cw - side_buffer], [height + 10, this.project.ch]);
        }
